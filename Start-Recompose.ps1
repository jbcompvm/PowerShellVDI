...
$pools = Get-QADObject -Service localhost -DontUseDefaultIncludedProperties -IncludedProperties $arrIncludedProperties -LdapFilter "(&(objectClass=pae-ServerPool)(pae-SVIVmParentVM=*$ParentVM))" | Sort-Object "pae-DisplayName" | Select-object Name, "pae-DisplayName", "pae-SVIVmParentVM" , "pae-SVIVmSnapshot", "pae-SVIVmSnapshotMOID", "pae-MemberDN", @{Name="pae-SVIVmDatastore";expression={$_."pae-SVIVmDatastore" -match "replica"}}
...
ForEach($Pool in $Pools)
{
    ...
    If($usedspace -lt $freespace)
	{
		...
		#Get-DesktopVM -pool_id TrainingPC | Send-LinkedCloneRecompose -schedule (Get-Date).Addminutes(5) -parentVMPath /CCMH/vm/Templates/TrainingVDI -parentSnapshotPath /TrainingPC -forceLogoff:$true -stopOnError:$false
